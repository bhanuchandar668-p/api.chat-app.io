...
import { LOGIN_DONE, PASSWORD_INVALID, SIGN_UP_DONE, USER_ALREADY_EXISTS, USER_NOT_FOUND, } from "../constants/app-messages.js";
...
export class AuthHandlers {
    signUpHandlers = factory.createHandlers(async (c) => {
        ...
        const validData = await validateReq("auth:signup", reqData, "Validation failed");
        const existedUser = await getSingleRecordByAColumnValue(users, "email", validData.email);
        if (existedUser) {
            throw new ConflictException(USER_ALREADY_EXISTS);
        }
        const hashedPassword = await argon2.hash(reqData.password);
        const userData = await saveSingleRecord(users, {
            ...reqData,
    ...
    signInHandlers = factory.createHandlers(async (c) => {
        const reqData = await c.req.json();
        const validData = await validateReq("auth:signin", reqData, "Validation failed");
        const user = await getSingleRecordByAColumnValue(users, "email", validData.email);
        if (!user) {
            throw new NotFoundException(USER_NOT_FOUND);
        }
        const isValidPassword = await argon2.verify(user.password, validData.password);
        if (!isValidPassword) {
