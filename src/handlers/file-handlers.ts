...
export class FileHandlers {
  uploadFileHandlers = factory.createHandlers(async (c) => {
    ...
    const validData = await validateReq<ValidatedFileUpload>(
      "file:upload",
      req,
    ...
    const presignedUrl = await s3DataService.getPresignedUrl(
      fileKey,
      urlOptions
    );
  ...
  downloadFileHandlers = factory.createHandlers(async (c) => {
    ...
    const validData = await validateReq<ValidatedFileDownload>(
      "file:download",
      req,
